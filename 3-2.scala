...
def getPositions(list: List[String]) = {
  list.foldLeft(List(Position(0, 0))) {
    case (positions, x) =>
      ...
      (1 to x.tail.toInt).foldLeft(positions) {
        case (positions, _) => positions :+ operation(positions.last)
      }
  }
}

val positions1 = getPositions(inputList1).tail
val positions2 = getPositions(inputList2).tail

val intersections = positions1.intersect(positions2)

intersections.map(p => {
  positions1.indexOf(p) + positions2.indexOf(p) + 2 //+2 since index starts with 0
}).min
