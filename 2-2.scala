...
def run(list: List[Int]) = {
    @tailrec
    def iterate(l: List[Int], pointer: Int): List[Int] =
        l(pointer) match {
            case 99 => l
            case 1 =>
                val updatedList = l.patch(l(pointer+3), List(l(l(pointer+1))+l(l(pointer+2))), 1)
                iterate(updatedList, pointer + 4)
            case 2 =>
                val updatedList = l.patch(l(pointer+3), List(l(l(pointer+1))*l(l(pointer+2))), 1)
                iterate(updatedList, pointer + 4)
            case _ => throw new RuntimeException(s"Unknown opcode")
        }
